...
public class Utils {
	 ...
	 */
	public static void scheduleAsync(final Reconnect reconnect, final Runnable runnable, long time, TimeUnit timeUnit) {
		ProxyServer.getInstance().getScheduler().schedule(reconnect, new Runnable() {
			@Override
			public void run() {
				ProxyServer.getInstance().getScheduler().runAsync(reconnect, runnable);
			}
		}, time, timeUnit);
	}
